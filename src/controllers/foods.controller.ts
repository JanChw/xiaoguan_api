...
export class FoodsController {
  public userService = new FoodService();

  @Get('/foods')
  @OpenAPI({ summary: 'Return a list of foods' })
  async getFoods () {
    const findAllFoods: Food[] = await this.userService.findAllFoods()
  ...
  async getFoodById (@Param('id') foodId: number) {
    const food: Food = await this.userService.findFoodById(foodId)
  ...
  async createUser (@Body() food: CreateFoodDto) {
    const createFoodData: Food = await this.userService.createFood(food)
  ...
  async updateFood (@Param('id') foodId: number, @Body() foodData: UpdateFoodPartialDto) {
    ...
    const updateFoodData: Food[] = await this.userService.updateFood(foodId, foodData)
  ...
  async deleteFood (@Param('id') foodId: number) {
    const deleteFoodData: Food[] = await this.userService.deleteFood(foodId)
